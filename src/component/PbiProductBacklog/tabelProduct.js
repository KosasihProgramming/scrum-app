...
import ModalCopyPbiProduct from "../CopyPbiProduct/modalCopy";
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const response = await axios({
          method: "DELETE",
          url:
            "http://103.181.182.230:6060/api/database/rows/table/636/" +
            id +
            "/",
          headers: {
  ...
  const handleEdit = async (judul, target, alasan, perspektif, jenis) => {
    try {
      ...
      if (!judul || !alasan || !jenis.value || !perspektif.value) {
        ...
        const response = await axios({
          method: "PATCH",
          url: `http://103.181.182.230:6060/api/database/rows/table/636/${idData}/?user_field_names=true`,
          headers: {
            Authorization: "Token R5XKLhkMz3enZ7nfVvRBJhs4IjbApdVw",
            "Content-Type": "application/json",
          },
